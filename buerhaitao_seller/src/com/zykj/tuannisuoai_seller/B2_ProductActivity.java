...
import com.zykj.tuannisuoai_seller.adapter.B5_10_MyCollectionAdapter;
import com.zykj.tuannisuoai_seller.base.BaseActivity;
import com.zykj.tuannisuoai_seller.utils.HttpUtils;
import com.zykj.tuannisuoai_seller.utils.MyListView;
 ...
 *
 */
public class B2_ProductActivity extends BaseActivity implements IXListViewListener,B5_10_MyCollectionAdapter.GetMap_id{
...
//	RelativeLayout rl_delete;//删除按钮
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		adapter = new B5_10_MyCollectionAdapter(B2_ProductActivity.this,data,isEdit,key,this);
		...
		HttpUtils.getFavoriteProduct(res_getFavoriteProduct, getSharedPreferenceValue("key"),page+"",curpage+"");
		tagProduct = "1";
		tagStore   = "0";
		//全选按钮
			...
			public void onCheckedChanged(CompoundButton arg0, boolean isChecked) {
				...
				if (isChecked) //选中
				{
					for (int i = 0; i < B5_10_MyCollectionAdapter.data.size(); i++) {
						...
						B5_10_MyCollectionAdapter.map_commonid.put(i+"", B5_10_MyCollectionAdapter.data.get(i)
								.get("goods_commonid"));
...
//				adapter_store = new B5_10_MyCollectionAdapter(B2_ProductActivity.this,data_store,isEdit,key,this);
...
//			}
		...
		case R.id.ll_selling://出售中
			...
			tagProduct = "1";
			tagStore   = "0";
			tv_chushouzhong.setTextColor(Color.parseColor("#FF5500"));
...
//			RequestDailog.showDialog(this, "正在加载数据，请稍后");
		...
		case R.id.ll_outlet://已下架
			...
			tv_yixiajia.setTextColor(Color.parseColor("#FF5500"));
...
//			listview.setAdapter(adapter_store);
			...
			adapter = new B5_10_MyCollectionAdapter(B2_ProductActivity.this,data,isEdit,key,this);
			listview.setAdapter(adapter);
			HttpUtils.getFavoriteStore(res_getFavoriteStore, getSharedPreferenceValue("key"),page+"",curpage+"");
			break;
		case R.id.btn_xiajia://点击下架or上架
			...
			commonid_serilize = commonid_serilize.substring(0,commonid_serilize.length()-1);
			if (tagProduct=="1") //出售中的商品请求下架商品接口
			{
				HttpUtils.goods_unshow(res_goods_unshow, getSharedPreferenceValue("key"), commonid_serilize.trim());
			}
			else //已下架的商品请求上架商品接口
			{
				HttpUtils.goods_show(res_goods_unshow, getSharedPreferenceValue("key"), commonid_serilize.trim());
			}
			break;
		...
		case R.id.btn_delete://点击删除
			...
			commonid_serilize1 = commonid_serilize1.substring(0,commonid_serilize1.length()-1);
			HttpUtils.goods_delete(res_goods_unshow, getSharedPreferenceValue("key"), commonid_serilize1.trim());
			break;
			...
			if (error==null)//成功
			{
				try {
					...
					for (int i = 0; i < array.length(); i++) {
						...
						map.put("goods_salenum", jsonItem.getString("goods_salenum"));//总销量
						map.put("goods_commonid", jsonItem.getString("goods_commonid"));
						data.add(map);
					}
					adapter.notifyDataSetChanged();
				} 
