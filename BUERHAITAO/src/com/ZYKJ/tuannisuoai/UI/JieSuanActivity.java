...
public class JieSuanActivity extends BaseActivity {
	...
	public void onClick(View v) {
		...
		case R.id.tv_jiesuanqueren:
			...
			// HttpUtils.getBuySecond(res_BuySecond,key,"1",allcheckinfo,address_id,null,myJsonObject,myJsonObject1,"1");
			...
			if (tv_zffs.getText().toString().equals("微信支付")) {
				...
				try {
					...
					JSONObject myJsonObject1 = new JSONObject(pay_type);
					if (check>0) {
						HttpUtils.getBuySecond(res_BuySecond, key, "1",
								allcheckinfo, address_id, null, myJsonObject,
								myJsonObject1, "1","android");
					}else {
						HttpUtils.getBuySecond(res_BuySecond, key, "1",
								allcheckinfo, address_id, null, myJsonObject,
								myJsonObject1, "0","android");
					}
				} catch (JSONException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				...
				try {
					JSONObject myJsonObject = new JSONObject(dlyo_pickup_typ);
					JSONObject myJsonObject1 = new JSONObject(pay_type);
					HttpUtils.getBuySecond(res_BuySecond,key,"1",allcheckinfo,address_id,null,myJsonObject,myJsonObject1,"0","android");
				} catch (JSONException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
		...
		public void onSuccess(int statusCode, Header[] headers,
			...
			if (error == null)// 成功
			{
				try {
					...
					JSONArray order_group_list = datas
							.getJSONArray("order_group_list");//
					Tools.Log("order_group_list=" + order_group_list);
					for (int j = 0; j < order_group_list.length(); j++) {
						Map<String, Object> map = new HashMap();
						JSONObject order_group_list1 = (JSONObject) order_group_list
								.get(j);
						JSONArray order_list = order_group_list1
								.getJSONArray("order_list");
						JSONObject order_list1 = ((JSONObject) order_list
								.get(0));
					...
					Tools.Notic(this, "您已经付款成功", new OnClickListener() {
						...
						public void onClick(View arg0) {
							...
							Intent intent_toMy = new Intent(
									JieSuanActivity.this, B5_MyActivity.class);
